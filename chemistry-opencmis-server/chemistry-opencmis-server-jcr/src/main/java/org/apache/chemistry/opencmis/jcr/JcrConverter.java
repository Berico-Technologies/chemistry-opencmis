...
public final class JcrConverter {
    ...
    private static boolean isMultiple(Node n, String propertyName) throws RepositoryException {
        boolean result = false;
        boolean found = false;

        NodeType primaryType = n.getPrimaryNodeType();

        for(PropertyDefinition pd : primaryType.getPropertyDefinitions()){
            if(pd.getName().equals(propertyName)){
                found = true;
                result = pd.isMultiple();
                break;
            }
        }
        ...
        if(!found){
            ...
            for(NodeType mixin : n.getMixinNodeTypes()){
                for(PropertyDefinition pd : mixin.getPropertyDefinitions()){
                    if(pd.getName().equals(propertyName)){
                        found = true;
                        result = pd.isMultiple();
                        break;
